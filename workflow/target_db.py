...
def build_update_sql(updates: list[dict[str, str]], placeholder: str = "?") -> str:
    assignments = ", ".join(f"{quote_identifier(item['db_column'])} = {placeholder}" for item in updates)
    return f"UPDATE {target_table()} SET {assignments} WHERE {target_key_column()} = {placeholder}"


def build_filled_update_preview(contract_number: str, updates: list[dict[str, str]]) -> str:
    assignments = ", ".join(f"{quote_identifier(item['db_column'])} = {sql_literal(item['value'])}" for item in updates)
    return f"UPDATE {target_table()} SET {assignments} WHERE {target_key_column()} = {sql_literal(contract_number)}"


def update_target_contract(contract_number: str, updates: list[dict[str, str]]) -> TargetResult:
    db = target_db()
    if db == "postgres":
        return update_postgres(contract_number, updates)
    if db in {"odbc", "db2", "ibmi"}:
        return update_odbc(contract_number, updates)
    return update_sqlite(contract_number, updates)


def update_sqlite(contract_number: str, updates: list[dict[str, str]]) -> TargetResult:
...
def update_postgres(contract_number: str, updates: list[dict[str, str]]) -> TargetResult:
    import psycopg

    sql = build_update_sql(updates, "%s")
    params = [item["value"] for item in updates] + [contract_number]
...
def update_odbc(contract_number: str, updates: list[dict[str, str]]) -> TargetResult:
    import pyodbc
